 ...
 */
public class Diamond extends TexturedObject implements ICollidable {
    private Rectangle bounds;

    public Diamond(float x, float y, float speed) {
        super("diamond.png", x, y, speed);  
 ...
 // Static method to create an obstacle with randomized position
    public static Diamond createDiamond(float spawnableWidth, float spawnableHeight, float speed) {
        Random random = new Random();
        
        //Calculate spawnable area for y-position
        float entityHeight = new Diamond(0, 0, 0).getHeight();  // Use getHeight() to get the height dynamically
        ...
        return new Diamond(xPosition, yPosition, speed);
    }
